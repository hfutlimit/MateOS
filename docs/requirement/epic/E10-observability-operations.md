...
| **E6**（v0.6 补，DM-I10） | **`PERMISSION_GRANTED` / `PERMISSION_REVOKED` / `PERMISSION_OVERRIDE_CHANGED`** —— 权限覆盖的增删改与 `perm.changed` 失效事件同源写入；**权限变更必须可审计**（谁在什么 scope 对谁改了哪个 key 的 effect） |
